...
#   ####################################################################
   ...
   ######### MODEL FORMULAS
       ...
       #smooth years
        smy<-smoothCon(s(YearCollected, bs="cr", k=kyears, fx=TRUE), data=df.tmp, absorb.cons=TRUE)[[1]]
        ...
        #smooth day of year
        sdy<-smoothCon(s(doy, bs="cr", k=kdays, fx=TRUE), data=df.tmp, absorb.cons=TRUE)[[1]]
...
###### RUN ANALYSIS
        ...
        #Run nbinomial, poisson, zip model. Select the model with the lowest DIC.                
            ...
            top.model<-try(inla(index.gam, family = family, data = df.tmp,
                                control.predictor = list(compute = TRUE), control.compute = list(dic=TRUE, config = TRUE), lincomb=lcs, verbose =TRUE), silent = T)
            
            top.modelS<-try(inla(index.gamS, family = family, data = df.tmp,
                                control.predictor = list(compute = TRUE), control.compute = list(dic=TRUE, config = TRUE), lincomb=lcs, verbose =TRUE), silent = T)
...
# ANNUAL INDICES: GENERATED FOR FULL TIME PERIOD ONLY
  ...
  # Calculate abundance indices using the posterior sample of the model
            ...
            #Posterior GAM estimate with Year effect (iid)
              ...
              #Posterior GAM estimate smoothed (not year effect)     
                  ...
                  for(g in 1:nyears){
                    mn.yr1[g,1]<-mean(pred.yr[,g], na.rm=TRUE)
  ...
  # Run LOESS function
   ...
   mn.yr2<-mn.yr2 %>% select(results_code, area_code, year, season, period, species_code, species_id, index, SD, upper_ci, lower_ci, indexloess, meanObs)               
...
##End-point trends for all years, 10-years, and 3-generations (min 10 years)
                 ...
                 write.trend<-trend.out %>% select(results_code, version, area_code, species_code, species_id, season, period, years, min_year, max_year, Trend, Trend_Q_0.025, Trend_Q_0.95, sd, percent_change, Width_of_Credible_Interval, model_type, index_type)
...
#Estimate the slope trend base on the BBS slope trend function (saved in setup file)
               
                # Summary of the GAM smooth on year
                 wy=c(y1:y2)
                 ne = log(pred.yr2[,wy]) #these are the smoothed indices
                 m =  t(apply(ne,1,FUN = bsl)) #run through bbsbayes slope trend model
                 sl.t = as.vector((exp(m)-1)*100)
                 
                 #include output in the table
                 trend.out$Trend_slope<-median(sl.t, na.rm=TRUE)
                 trend.out$Trend_slope_Q_0.025<-quantile(sl.t, prob=0.025)
                 trend.out$Trend_slope_Q_0.95<-quantile(sl.t, prob=0.950)
                 trend.out$sd_slope<-sd(sl.t, na.rm=TRUE)
                 trend.out$percent_change_slope= ((1+(trend.out$Trend_slope/100))^(1/(Y2-Y1))-1)*100
                #trend.out$percent_change_slope=100*(median(m)-1)
                 ...
                 write.trend<-NULL
                 write.trend<-trend.out %>% select(results_code, version, area_code, species_code, species_id, season, period, years, min_year, max_year, Trend_slope, Trend_slope_Q_0.025, Trend_slope_Q_0.95, sd, percent_change_slope, Width_of_Credible_Interval, model_type, index_type)
